...
fn bench_evm_execution(c: &mut Criterion) {
    ...
    // 从环境变量获取配置
    let datadir = std::env::var("PEVM_DATADIR").unwrap_or_else(|_| "d:\\reth2k".to_string());
    let log_dir = std::env::var("PEVM_LOG_DIR").unwrap_or_else(|_| "./bench_logs".to_string());
    let pevm_exe =
        std::env::var("PEVM_EXE").unwrap_or_else(|_| "./target/release/pevm.exe".to_string());
...
fn execute_blocks_via_cli(
    ...
    if use_log {
        cmd.arg("--use-log").arg("on").arg("--log-dir").arg(log_dir);
    }
