...
| + the profile-guided `gmp` build, on a quiet machine | **19.7 min** | 297,481 s | **every block, 0 failures, 0 tasks aborted**; 266.4 Ggas/s, 3.13M TPS; the same build with a light sync node sharing the disk for its first minutes: 20.0 min |
