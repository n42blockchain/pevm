...
RUSTFLAGS="-Cprofile-generate=$(realpath "$DATA")" cargo build --release --target-dir target-pgo
echo "== training"
...
RUSTFLAGS="-Cprofile-use=$(realpath target-pgo/merged.profdata)" cargo build --release --target-dir target-pgo
