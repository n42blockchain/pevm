...
impl<C: ChainSpecParser<ChainSpec = ChainSpec>> Command<C> {
    ...
    pub async fn execute<N: CliNodeTypes<ChainSpec = C::ChainSpec, Primitives = EthPrimitives>>(
        ...
        let (in_memory_state_root, in_memory_updates) =
            StateRoot::overlay_root_with_updates(provider.tx_ref(), &hashed_post_state_sorted)?;
        ...
        let (state_root, incremental_trie_updates) =
            StateRoot::incremental_root_with_updates(&provider_rw, block.number..=block.number())?;
