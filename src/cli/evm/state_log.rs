        ...
        #[cfg(unix)]
        {
            unsafe {
                ...
                if file_size < WILLNEED_THRESHOLD {
                    ...
                    info!(
                        "madvise: SEQUENTIAL | WILLNEED (file size: {:.1} MB < 1GB threshold)",
...
impl MmapStateLogDatabase {
    ...
    fn migrate_v1_to_v2(old_path: &Path, data_path: &Path, index_path: &Path) -> eyre::Result<()> {
        ...
        info!(
            "Old file: {} blocks, index_offset={}",
            block_count, index_offset
        );
        ...
        info!(
            "Migration complete: {} bytes data, {} bytes index",
            data_section.len(),
            index_section.len()
        );
    ...
    fn open_existing(data_path: &Path, index_path: &Path, write_mode: bool) -> eyre::Result<Self> {
        ...
        if skipped_invalid > 0 {
            warn!(
                "Skipped {} invalid index entries (corrupted data)",
                skipped_invalid
            );
        }
        ...
        let mmap_data = if write_mode {
            info!(
                "MmapStateLogDatabase opened in WRITE mode (v2): {} blocks, data_end={} (no mmap)",
                index.len(),
                data_end
            );
            None
            ...
            info!(
                "MmapStateLogDatabase opened in READ mode (v2): {} blocks, data_end={}",
                index.len(),
                data_end
            );
            Some(mmap)
    ...
    pub(crate) fn write_block_log(
        ...
        if self.pending_writes.len() >= 10000 {
            info!(
                "Auto-flushing {} pending blocks to prevent memory growth",
    ...
    /// 自动跳过已存在且有效的块（避免重复写入）
    /// 对于损坏的块，会覆盖重新生成
    pub(crate) fn write_block_logs_batch(
        &mut self,
        batch: &[(u64, Vec<ReadLogEntry>)],
    ) -> eyre::Result<()> {
        ...
        if skipped > 0 && skipped >= batch.len() / 2 {
            // 只在跳过大量块时输出日志（避免日志泛滥）
            warn!(
                "Skipped {} existing blocks out of {} in batch",
                skipped,
                batch.len()
            );
        }
    ...
    pub fn flush(&mut self) -> eyre::Result<()> {
        ...
        info!(
            "MmapStateLogDatabase flushed (v2): {} new blocks, {} total blocks, data_end={}",
            pending_count,
            self.index.len(),
            self.data_end
        );
    ...
    pub fn is_block_corrupted(&self, block_number: u64) -> bool {
        if let Some((offset, length)) = self.index.get(&block_number) {
            ...
            if let Some(ref mmap) = self.mmap_data {
                ...
                if *length >= 8 {
                    let count =
                        u64::from_le_bytes(mmap[start..start + 8].try_into().unwrap_or([0u8; 8]));
                    // 条目数量应该合理（不超过 100 万）
                    if count > 1_000_000 {
                        return true; // 数据格式损坏
                    }
    ...
    pub fn mark_block_corrupted(&mut self, block_number: u64) {
        if self.index.remove(&block_number).is_some() {
            warn!(
                "Marked block {} as corrupted, will be regenerated",
                block_number
            );
            self.dirty = true;
        }
...
/// 分析损坏的索引条目，尝试找出规律
fn analyze_corrupted_entries(index_file_path: &Path, data_end: u64) -> eyre::Result<()> {
    ...
    if let Some(first) = first_invalid {
        ...
        info!(
            "Last valid data offset: {} bytes ({:.2} GB)",
            last_valid_offset,
            last_valid_offset as f64 / 1024.0 / 1024.0 / 1024.0
        );
        info!(
            "Data file size: {} bytes ({:.2} GB)",
            data_end,
            data_end as f64 / 1024.0 / 1024.0 / 1024.0
        );

        if last_valid_offset < data_end {
            let orphaned_data = data_end - last_valid_offset;
            info!(
                "Orphaned data (no index): {} bytes ({:.2} GB)",
                orphaned_data,
                orphaned_data as f64 / 1024.0 / 1024.0 / 1024.0
            );
...
impl MmapStateLogDatabase {
    ...
    pub fn repair_log(
        ...
        info!(
            "Total entries read: {}, valid: {}, invalid: {}",
            total_entries,
            all_entries.len(),
            invalid_entries
        );
        ...
        if truncated_entries > 0 {
            info!(
                "Truncated {} entries with block number > {}",
                truncated_entries, expected_end
            );
        }
        ...
        info!(
            "Validating data integrity for {} blocks...",
            unique_entries.len()
        );
        ...
        if !corrupted_blocks.is_empty() {
            warn!(
                "Found {} corrupted blocks with invalid data",
                corrupted_blocks.len()
            );
            if corrupted_blocks.len() > 20 {
                warn!(
                    "... and {} more corrupted blocks",
                    corrupted_blocks.len() - 20
                );
            }
        }

        info!(
            "Data validation complete: {} valid, {} corrupted",
            valid_entries.len(),
            corrupted_blocks.len()
        );
        ...
        for block_num in check_start..=check_end {
            if !block_set.contains(&block_num) {
                ...
                if missing_blocks.len() >= 1000 {
                    warn!(
                        "Found {} missing/corrupted blocks, stopping check...",
                        missing_blocks.len()
                    );
                    break;
                }
        ...
        for block_num in corrupted_blocks.iter() {
            if !missing_blocks.contains(block_num)
                && *block_num >= expected_start
                && *block_num <= expected_end
            {
                missing_blocks.push(*block_num);
            }
        }
        ...
        if has_missing_blocks {
            ...
            warn!(
                "Found {} missing/corrupted blocks in range {} - {}",
        ...
        let mut new_data_file =
            BufWriter::with_capacity(4 * 1024 * 1024, File::create(&new_data_path)?);
        let mut new_index_file =
            BufWriter::with_capacity(1024 * 1024, File::create(&new_index_path)?);
        ...
        let mut valid_entries_for_write: Vec<(u64, u64, u32)> =
            Vec::with_capacity(unique_entries.len());
        ...
        for (block_number, old_offset, length) in unique_entries.iter() {
            ...
            if !is_data_valid {
                warn!(
                    "Block {} data is corrupted (invalid format), skipping",
                    block_number
                );
                skipped_corrupted += 1;
                continue;
        ...
        if skipped_corrupted > 0 {
            warn!(
                "Skipped {} corrupted blocks during compaction",
                skipped_corrupted
            );
        }
        ...
        info!(
            "  - Invalid index entries (bad offset/length): {}",
            invalid_entries
        );
        info!(
            "  - Corrupted data blocks (bad content): {}",
            corrupted_count
        );
        info!(
            "  - Valid entries after validation: {}",
            unique_entries.len()
        );
        info!("  - Duplicate entries removed: {}", duplicate_entries);
        info!(
            "  - Truncated entries (> {}): {}",
            expected_end, truncated_entries
        );
        if skipped_corrupted > 0 {
            info!(
                "  - Additional corrupted blocks during compaction: {}",
                skipped_corrupted
            );
        }
        ...
        info!(
            "  - Block range: {} - {}",
            final_min_block.unwrap_or(0),
            final_max_block.unwrap_or(0)
        );
        info!(
            "  - Original size: {:.2} GB",
            data_end as f64 / 1024.0 / 1024.0 / 1024.0
        ...
        info!(
            "  - Compacted size: {:.2} GB",
            new_data_size as f64 / 1024.0 / 1024.0 / 1024.0
        );
        info!(
            "  - Space saved: {:.2} GB ({:.1}%)",
        ...
        info!(
            "  - Backup files: {:?}, {:?}",
            backup_data_path, backup_index_path
        );
        ...
        Ok(RepairResult {
            ...
            error_message: if total_missing > 0 {
                Some(format!(
                    "{} missing/corrupted blocks need to be regenerated",
                    total_missing
                ))
            } else {
                None
    ...
    pub fn rebuild_index(
        ...
        info!(
            "Data file size: {} bytes ({:.2} GB)",
            data_file_size,
            data_file_size as f64 / 1024.0 / 1024.0 / 1024.0
        );
        ...
        if index_file_path.exists() {
            ...
            if index_file.read_exact(&mut header).is_ok() {
                ...
                if magic == Self::MAGIC_INDEX {
                    ...
                    info!(
                        "Read {} index entries, {} invalid",
                        total_index_entries, invalid_index_entries
                    );
                } else {
            ...
            return Err(eyre::eyre!(
                "Index file not found and cannot recover without it. \
        ...
        for (block_number, offset, length) in all_entries.iter() {
            ...
            if end > data_mmap.len() {
                ...
                if corrupted_blocks.len() <= 20 {
                    warn!(
                        "Corrupted block {} (data extends beyond file)",
                        block_number
                    );
                }
                continue;
        ...
        // 检查缺失块
        info!(
            "Checking for missing blocks in range {} - {}...",
            expected_start, expected_end
        );
        let block_set: std::collections::HashSet<u64> =
            valid_entries.iter().map(|(bn, _, _)| *bn).collect();
        ...
        // 写入新的索引文件
        info!(
            "Writing new index file with {} valid entries...",
            valid_entries.len()
        );
        ...
        if total_missing > 0 {
            warn!(
                "=== Missing/Corrupted blocks to regenerate: {} ===",
                total_missing
            );
