...
use crate::{
    block::{
        state_changes::{add_increment, post_block_balance_increments_ordered},
        BlockExecutionError, BlockExecutionResult,
    ...
    <R::Transaction as TransactionEnvelope>::TxType: Send + 'static,
{
    ...
    fn finish(
        ...
        // Ordered, not the map version: a keyless witness records state reads
        // as a bare stream, so the order these beneficiaries are read in has to
        // be reproducible across runs.
        let mut balance_increments = post_block_balance_increments_ordered(
            &self.spec,
            self.evm.block(),
            self.ctx.ommers,
            ...
            .transitions_at_block(self.evm.block().number().saturating_to())
        {
            ...
            add_increment(
                ...
                dao_fork::DAO_HARDFORK_BENEFICIARY,
                drained_balance,
            );
        }
        // increment balances
